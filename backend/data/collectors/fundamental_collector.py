...
def _fetch_single_stock(kode_saham: str, market: str = "IDX") -> dict[str, Any] | None:
    """
    ...
    Args:
        kode_saham: Kode saham tanpa suffix (contoh: "BBCA", "SNPS")
        market: "IDX" (default, pakai suffix .JK) | "NASDAQ" | "NYSE" | "ETF"

    Returns:
        Dictionary data fundamental, atau None jika gagal
    """
    # Format ticker untuk Yahoo Finance: market-aware (BBCA -> BBCA.JK, SNPS -> SNPS)
    ticker_symbol = get_yf_symbol(kode_saham, market)
...
async def collect_fundamental(
    ...
    """
    Ambil data fundamental untuk sekumpulan saham dari Yahoo Finance.
    ...
    Args:
        ...
        market: Market asal SEMUA kode di list ini ("IDX" | "NASDAQ" | "NYSE" | "ETF").
                Jika list berisi campuran market, panggil fungsi ini terpisah per
                grup market (lihat workers.scrape_fundamental_job).
    ...
    logger.info(
        ...
        f"untuk {len(kode_saham_list)} saham (market={market})..."
    )
    ...
    for i, kode in enumerate(kode_saham_list):
        ...
        try:
            data = await asyncio.to_thread(_fetch_single_stock, kode_clean, market)
...
async def collect_fundamental_batch(
    ...
    Args:
        ...
        market: Market asal SEMUA kode di list ini (lihat collect_fundamental)
    ...
    logger.info(
        ...
        f"market={market}, batch_size={batch_size}"
    )
    ...
    for batch_num in range(0, len(kode_saham_list), batch_size):
        ...
        batch_results = await collect_fundamental(batch, market=market)
...
def _fetch_single_stock_last_price(kode_saham: str, market: str = "IDX") -> dict[str, Any] | None:
    """
...
async def collect_last_prices(
    ...
    Args:
        kode_saham_list: List kode saham
        market_map: Optional dict {kode: market} untuk watchlist campuran
                    (mis. IDX + NASDAQ sekaligus). Kode yang tidak ada di
                    dict ini diasumsikan "IDX" (perilaku lama, tidak berubah).
    """
    ...
    for kode in kode_saham_list:
        kode_clean = kode.strip().upper()
        market = market_map.get(kode_clean, "IDX")
        data = await asyncio.to_thread(_fetch_single_stock_last_price, kode_clean, market)
        if data:
            results.append(data)
