...
async def job_weekly_scoring() -> None:
    ...
    try:
        ...
        logger.info(f"🚀 [SCHEDULER] Scoring {len(watchlist)} saham watchlist...")
...
async def job_earnings_and_rally_check() -> None:
    """
    Job 6: Mengecek jadwal rilis laporan keuangan (H-3/H-1) dan rally streak
    (hijau berturut-turut) untuk setiap emiten watchlist, lalu membuat
    notifikasi (Alert) kalau relevan. Dijalankan 1x sehari jam 08:00 WIB,
    setelah data fundamental & harga harian selesai diperbarui.
    """
    logger.info("⏱️ [SCHEDULER] Menjalankan Job 6: Cek Earnings & Rally Streak...")
    try:
        await check_earnings_and_rally_job()
        logger.info("✅ [SCHEDULER] Job 6: Cek Earnings & Rally Streak selesai.")
    except Exception as e:
        logger.error(f"❌ [SCHEDULER] Job 6: Gagal menjalankan cek earnings & rally streak: {e}")
...
def setup_scheduler() -> None:
    ...
    # Job 6: Setiap hari jam 08:00 WIB untuk cek jadwal earnings & rally streak
    scheduler.add_job(
        job_earnings_and_rally_check,
        CronTrigger(hour=8, minute=0),
        id="earnings_and_rally_check_job",
        name="Daily Earnings Schedule & Rally Streak Check",
        replace_existing=True,
    )
