...
Tabel:
    - Saham           : Watchlist emiten yang dipantau penuh (scoring, scrape, alert)
    - SimbolReferensi : Universe ringan untuk search (ribuan simbol, tidak disentuh scheduler)
...
class Saham(Base):
    ...
    market: Mapped[str] = mapped_column(
        ...
        default="IDX",
        server_default="IDX",
        comment="Pasar tempat saham diperdagangkan: IDX, NASDAQ, NYSE, ETF, dst. "
                "Dipakai untuk menentukan suffix ticker yfinance & sumber berita/makro.",
    )
    is_watchlist: Mapped[bool] = mapped_column(
        ...
        default=True,
        server_default="true",
        comment="True = ikut dipantau scheduler (scoring mingguan, scrape fundamental "
                "harian, update harga 30 menit, dll). False = hanya hasil 'Analyze' "
                "on-demand sekali jalan, TIDAK ikut job terjadwal sampai user eksplisit "
...
# ============================================================
# Tabel 1b: SimbolReferensi — Universe Pencarian (ringan, terpisah dari watchlist)
# ============================================================

class SimbolReferensi(Base):
    """
    Daftar referensi ribuan simbol (NASDAQ, ETF, dst.) khusus untuk fitur
    search/autocomplete di frontend.

    SENGAJA dipisah dari tabel `Saham`: tabel ini TIDAK punya relationship
    ke Fundamental/Berita/ScoringMingguan/Alert dan TIDAK PERNAH disentuh
    oleh scheduler (lihat workers.py & scheduler.py — semua job hanya query
    `Saham` yang `is_watchlist=True`). Jadi mengisi tabel ini dengan 4000+
    baris tidak menambah beban job terjadwal sama sekali — cuma data ringan
    untuk di-ILIKE saat user mengetik di search bar.

    Saat user menemukan simbol di sini dan klik "Analyze" atau "Tambah ke
    Watchlist", baru baris yang relevan dibuatkan/disinkronkan ke tabel
    `Saham` (lihat workers.get_or_create_saham / set_watchlist_status).
    """

    __tablename__ = "simbol_referensi"
    __table_args__ = (
        Index("ix_simbol_referensi_nama", "nama"),
    )

    kode: Mapped[str] = mapped_column(
        String(15),
        primary_key=True,
        comment="Kode/ticker simbol (mis. SNPS, QQQ, BBCA)",
    )
    nama: Mapped[str] = mapped_column(
        String(255),
        nullable=False,
        comment="Nama perusahaan/fund",
    )
    market: Mapped[str] = mapped_column(
        String(20),
        nullable=False,
        default="IDX",
        comment="IDX, NASDAQ, NYSE, ETF, dst.",
    )
    tipe: Mapped[str] = mapped_column(
        String(20),
        nullable=False,
        default="STOCK",
        comment="STOCK atau ETF",
    )
    exchange: Mapped[str | None] = mapped_column(
        String(50),
        nullable=True,
        comment="Nama exchange asal data (mis. NASDAQ, NYSE Arca)",
    )
    ...
    def __repr__(self) -> str:
        return f"<SimbolReferensi(kode='{self.kode}', market='{self.market}')>"
...
class Alert(Base):
    """
    Data alert/notifikasi terkait saham.

    Awalnya hanya dipakai oleh alert_agent untuk fluktuasi sentimen (> 10 poin),
    sekarang juga dipakai untuk jadwal rilis laporan keuangan (earnings) dan
    rally streak (harga hijau berturut-turut). Kolom `jenis` membedakan
    tipe-tipe ini supaya frontend bisa render ikon/style berbeda per jenis.
    """
    ...
    jenis: Mapped[str] = mapped_column(
        ...
        nullable=False,
        default="sentimen",
        server_default="sentimen",
        comment="Jenis alert: 'sentimen' | 'earnings' | 'rally_streak'",
    )
