...
async def init_database() -> None:
    ...
    try:
        async with engine.begin() as conn:
            ...
            # Migrasi: kolom market & is_watchlist di tabel saham (untuk DB yang
            # sudah berjalan sebelum fitur multi-market/search ditambahkan).
            # Default 'IDX' / true menjaga 20 saham IDX existing tetap berperilaku
            # persis seperti sebelumnya (tetap dipantau scheduler tanpa perlu setup ulang).
            await conn.execute(text(
                "ALTER TABLE saham ADD COLUMN IF NOT EXISTS market VARCHAR(20) "
                "NOT NULL DEFAULT 'IDX';"
            ))
            await conn.execute(text(
                "ALTER TABLE saham ADD COLUMN IF NOT EXISTS is_watchlist BOOLEAN "
                "NOT NULL DEFAULT true;"
            ))

            # Migrasi: kolom jenis di tabel alert (untuk membedakan alert
            # sentimen / earnings / rally_streak). Default 'sentimen' menjaga
            # baris alert lama (dari fitur sentimen) tetap terklasifikasi benar.
            await conn.execute(text(
                "ALTER TABLE alert ADD COLUMN IF NOT EXISTS jenis VARCHAR(30) "
                "NOT NULL DEFAULT 'sentimen';"
            ))
