...
async def scrape_and_save_fundamental_for_emiten(kode: str, market: str = "IDX") -> dict[str, Any] | None:
    """
    Melakukan scraping data fundamental secara real-time untuk emiten tertentu
    dari Yahoo Finance (+ XBRL IDX jika market == "IDX"), lalu menyimpannya ke
    database PostgreSQL.
    """
    market_clean = normalize_market(market)
    logger.info(f"🔍 [On-Demand Fundamental] Mengambil data fundamental baru untuk {kode} (market={market_clean})...")
    try:
        ...
        raw_list = await collect_fundamental([kode], market=market_clean)
        if not raw_list:
        ...
        # 2. Fetch dari XBRL IDX — sumber ini KHUSUS laporan keuangan emiten IDX,
        #    jadi dilewati untuk saham non-IDX (mis. NASDAQ) supaya tidak
        #    membuang waktu memanggil API yang pasti tidak akan menemukan datanya.
        if market_clean == "IDX":
            try:
    ...
    if is_mock_session:
        ...
        async with async_session() as session:
            for k_code in daftar_saham:
                ...
                market_map[k_code] = normalize_market(s_obj.market if s_obj else None)
    ...
    for kode in daftar_saham:
        ...
        try:
            ...
            if not data_fundamental or (
                ...
                cleaned_fund = await scrape_and_save_fundamental_for_emiten(kode, market=market_map.get(kode, "IDX"))
            ...
            qoq_growth = 0.0
            market_kode = market_map.get(kode, "IDX")
            try:
                import yfinance as yf
                import pandas as pd
                ticker_symbol = get_yf_symbol(kode, market_kode)
            ...
            # perubahan_ihsg adalah pergerakan indeks IDX — tidak relevan untuk
            # saham non-IDX (mis. NASDAQ), jadi dinetralkan (selisih=0 -> skor 50)
            # supaya pergerakan bursa Indonesia tidak "bocor" ke skor sektor saham asing.
            perubahan_ihsg_efektif = perubahan_ihsg if market_kode == "IDX" else 0.0
            s_sektor = _skor_sektor(sektor, kinerja_sektoral, perubahan_ihsg_efektif)
